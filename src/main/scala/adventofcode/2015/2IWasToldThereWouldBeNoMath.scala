/** The elves are running low on wrapping paper, and so they need to submit an
  ...
  */
@main def wrappingPaperNeeded(): Int =
  val dimensions: Seq[String] = os.read.lines(os.pwd / "AOC" / "15DAY02.txt")
  val wrappingForEachPresent: Seq[Int] = dimensions.map[Int] {
    dimension =>
      ...
      val extraPaper = sides.min
      surfaceArea + extraPaper
  }

  val totalPaperNeeded = wrappingForEachPresent.sum
  println(s"Elves need $totalPaperNeeded total square feet of wrapping paper!")
  totalPaperNeeded
